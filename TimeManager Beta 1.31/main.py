...
######### Main Menu #########
    ...
    def graphFunction(self, totaltime, lei, com):
        if not lei and not com:
            return

        matplotlib.rcParams['figure.figsize'] = [22.0, 10.0]  # Increase the figure width
        # Δημιουργώ ένα subplot με 1 μόνο πίτα εαν έχει μονο ενός τύπου δραστηριότητες, αλλιώς δημιουργεί 2 πίτες
        fig, ax = plt.subplots(nrows=1, ncols=1 if len(lei) == 0 or len(com) == 0 else 2)
        plt.suptitle('Αναπαράσταση Δεδομένων')

        # Leisure
        if len(lei) != 0:
            # Δημιουργώ μια λίστα με τα ονόματα των δραστηριοτήτων ελεύθερου χρόνου
            leiData = [lei[i][1] for i in range(len(lei))]
            # Δημιουργώ μια λίστα με τoν χρόνο που παίρνει η κάθε δραστηριότητα επί του συνολικού χρόνου
            leiTime = [100.0 * (float(lei[i][4]) / totaltime) for i in range(len(lei))]
            colors_lei = sns.color_palette("RdBu", len(leiData))
            # Χρησιμοποιώ την μέθοδο pie για να σχεδιάσω την πίτα και περνάω ως ορίσματα τις 2 λίστες που έφτιαξα
            wedges, texts, autotexts = ax[0].pie(leiTime, labels=leiData, autopct='%1.2f%%', colors=colors_lei,
                                                 textprops={'fontsize': 8})
            ax[0].set_title("Δραστηριότητες Ελεύθερου Χρόνου", wrap=True,
                            fontsize=10)  # Set the title for the Leisure subplot
            plt.setp(autotexts, size=8, weight="bold")
            plt.setp(texts, size=8)

        # Commitment
        if len(com) != 0:
            # Δημιουργώ μια λίστα με τα ονόματα των υποχρεωτικών δραστηριοτήτων
            comData = [com[i][1] for i in range(len(com))]
            # Δημιουργώ μια λίστα με τoν χρόνο που παίρνει η κάθε δραστηριότητα επί του συνολικού χρόνου
            comTime = [100.0 * (float(com[i][4]) / totaltime) for i in range(len(com))]
            colors_com = sns.color_palette("PRGn", len(comData))
            if len(lei) != 0:
                wedges, texts, autotexts = ax[1].pie(comTime, labels=comData, autopct='%1.1f%%', colors=colors_com,
                                                     textprops={'fontsize': 8})
                ax[1].set_title("Δραστηριότητες Υποχρεώσεων", wrap=True,
                                fontsize=10)  # Set the title for the Commitment subplot
                plt.setp(autotexts, size=8, weight="bold")
                plt.setp(texts, size=8)
            else:
                wedges, texts, autotexts = ax.pie(comTime, labels=comData, autopct='%1.1f%%', colors=colors_com,
                                                  textprops={'fontsize': 8})
                ax.set_title("Δραστηριότητες Υποχρεώσεων", wrap=True,
                             fontsize=10)  # Set the title for the Commitment plot
                plt.setp(autotexts, size=8, weight="bold")
                plt.setp(texts, size=8)

        #  Κάνω wrap τα labels κάθε δραστηριότητας σε νέα γραμμή με μέγιστο δύο λέξεις ανά γραμμή
        for text in texts:
        ...
        plt.subplots_adjust(wspace=0.5)  # Προσαρμόζω το κενό μεταξύ των subplots
...
########## This Block here is for inserting a new user ##########
    ...
    def checkFirstPage(self):
        ...
        # The code below does some sort of defensive programming (Αμυντικός)
